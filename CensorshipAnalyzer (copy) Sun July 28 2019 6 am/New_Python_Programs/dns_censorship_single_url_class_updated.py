...
class DNS_CENSORSHIP:
	...
	def send_to_java(self, MESSAGE):
		...
		print("-->>Sent to java begin in dns_cens...")
	...
	def print1(self, message):
		...
		self.print_message(message.replace('\n', ''))
	...
	def send_to_database(self, domain_name_1, server_ip, max_ttl):
		...
		self.print1("Hop Count: " + censored_server_hop_count.__str__())
	...
	def dns_censorship_check(self, HOST):
			...
			if idLocal is 1:
				...
				for answer in answersFromLocal:
					...
					if answer.to_text() not in str(ipListLocal):
						...
						self.print1("Local:" + answer.to_text())
			elif idLocal is 2:
				nxdomainL = True
			elif idLocal is 3:
				countL = countL + 1
		...
		if not answer:
				...
				if idRemote is 1:
					...
					for answer in answersFromRemote:
						...
						if answer.to_text() not in str(ipListRemote):
							...
							self.print1("Remote:" + answer.to_text())
				elif idRemote is 2:
					nxdomainR = True
				elif idRemote is 3:
					countR = countR + 1
			...
			if answer:
				...
				if isLDSOk:
					self.print1(HOST + " is censored")
					self.report.is_censored = 1
				else:
					self.print1("Local Dns Server may be down")  ###########################
				if nxdomainL:
				...
				self.print1("DNS Resolution Unsuccessful")
				if nxdomainR and nxdomainL:
					self.print1(HOST + " is not censored")
					self.report.dns_description.is_nxDomain = 1
			...
			if not answer:
				...
				if not isOverlapped:
					if contain_bogon_ip:
						...
						if isOverlapped:
							self.print1(HOST + " is not censored(first 8-24 bit matched)")
							self.report.dns_description.is_first_8_to_24_bit_match = 1
						else:
							self.print1("Non-overlapped ip set")
							self.report.dns_description.is_non_overlapping_ip_list = 1

				else:
					self.print1(HOST + " is not censored")
					self.print1("Local Dns Server and Stubby Return Overlapped Ip Address")
...
if is_Main:  # run as main
	...
	dns_check.print1("Returned Report:")
	dns_check.print1(str_report)
