	...
	def tcp_handshake_check(self,HOST,iteration):
		...
		for ip in ipListRemote: 	#EACH IP
				...
				if statusTor:
					...
					for k in range(self.Iterate):
						...
						if statusLocal:
							...
							report.tcp_description.successful_iteration_number = k
							break
					...
					if not statusLocal:
						...
						if status:
							report.tcp_description.successful_iteration_number = self.Iterate
						else:
			...
			if not statusTor:	#Full connection failure via tor
				report.tcp_description.is_tor_connect_successfull = 0
